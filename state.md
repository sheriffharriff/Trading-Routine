...
```
last_run: 2026-09-03 09:36 ET 2-market-open-execution
week_of: 2026-08-31
...
- **✅ THE §2 CORE SLEEVE IS ESTABLISHED — the first real fill in this repo's history landed at
  09:36:21 ET today.** `core_established` is now `true` and the flag path in Step 3 will not run
  again. Fill: BUY VOO **99.046311231 @ $706.74**, notional $70,000.00, order
  `d177d8f0-cd0c-41bf-95c1-4772318265fd`, verified `"status": "filled", "terminal": true` before
  it was written to `trade_log.md`. Live equity was $100,000.00 at the open so the plan's $70,000
  notional was also 70% of live equity — no size adjustment needed. Post-fill sleeves at 09:36 ET:
  core **69.99%**, satellite 0.0%, cash **30.01%**, `core_in_band: true`,
  `rebalance_needed: false`, `rebalance_delta: 5.06` (residual from fractional-share rounding
  against a moving mark, immaterial and well inside the §2 65–75% band). **Do not re-run the
  bootstrap tomorrow, or the day after, or ever** — Step 3's whole point is that this path never
  fires again once the flag flips. Step 7's rebalance will only fire again if drift takes core
  outside 65–75%, which today's $5.06 residual does not.

- **⚠ `positions.md` now legitimately disagrees with the broker, exactly as pre-noted.** The
  ledger still reads *(none)* while `alpaca.py positions` returns one VOO row (99.046311231 @
  avg_entry 706.74). **This is not a discrepancy** — §5 exempts core from all four sell rules, so
  it deliberately has no thesis, no timing window, no `highest_close`. Every reconciliation note
  from tomorrow onward must check **satellite blocks** against satellite Alpaca positions, not the
  raw ledger against the raw broker. The first run that sees this and does not carry this line
  will read the mismatch as broken.

- **The §5.4 trailing stop is still not armed, and today's fill did not arm it.** Core is exempt
  from §5.4 by design. It arms the day the first *satellite* position opens, which is still not
  today. This is the twelfth consecutive run to record the distinction rather than assume it
  carried — the state ("not silently disabled, not yet armed") is unchanged by a core fill.
...
- **Week rollover checked this run: none due.** ISO Monday of 2026-09-03 (Thursday) is
  **2026-08-31**, which matches `week_of`. `new_positions_this_week` stays 0 — the §6 weekly cap
  is fully available at 0 of 3, and today's core bootstrap does **not** count against it (§6 caps
...
- **Cleared from carry-forward this run:** the entire pre-fill bootstrap block (resolved — the
  fill landed and was verified), the "size to live equity" instruction (resolved — done, and
  equity happened to equal the written notional), the `positions.md` will-disagree-tomorrow
  warning (resolved — that condition is now live and carries as its own line above), and the
  "trading is live as of today" preamble (no longer news; the first live fill is the artefact).
  All research carry-forward is retained because the funnel is rolling and the same names will
  recur.
