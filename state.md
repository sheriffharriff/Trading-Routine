...
- **⚠ HIGH-WATER MARKS WERE NOT UPDATED, AND NOTHING WAS SKIPPED. DO NOT BACKFILL — THERE IS NOTHING TO
  ...
  A SUBJECT, not thirty-one clean bills of health.** ⚠ **09-24's open adds a second, independent
...
- **⚠ TAPE FACTS THE NEXT RUN SHOULD NOT RE-DERIVE.** VOO **official** closes: **09-23 707.28**,
  ...
  **$30,000.00** flat. **At the 09-24 OPEN (09:36, in-session): equity $99,808.83, core $69,808.83 =
  69.94%, `rebalance_delta` +$57.35.** **NO REBALANCE DUE** — §2 rebalances at the **65/75 band edge**,
  not to the exact target, so no delta inside the band is an action at any size, and the core sits
  **4.94 points** from the nearest edge. ⚠ **The delta stayed POSITIVE for a second consecutive run and
  shrank again (+67.16 → +57.35) — that is the core tracking the tape, not drift.** **Fortieth
  consecutive run inside the 0.63-point range 69.59–70.22.** **Audit any superlative here before
  repeating it.**
...
- **⚠⚠ NEWEST AND MOST USEFUL, 09-24: `lastday_price` HAS NO RELIABLE STRUCTURE AT ALL, AND THE TWO
  ...
  *"today's official close is 707.28 — if `lastday_price` still reads 712.78 tomorrow morning it is
  ...
  real volume arrives; **it does not.** ⚠ **CONCRETE, FALSIFIABLE PREDICTION FOR THE NEXT RUN, written
...
- **⚠ GNRC IS NOT YOURS TO LOOK AT — TWENTY-ONE CONSECUTIVE REFUSALS, AND THE TWENTY-FIRST WAS FREE.**
...
- **⚠ CORE VOO IS NEVER STAMPED WITH A `highest_close` — FORTY-NINE RUNS.** §5 exempts core from all
  ...
  09-24 OPEN is by contrast a WEAK instance and is recorded as such — it pulled no close and held no
  official price, so the refusal cost nothing.** ⚠ **"Nothing
